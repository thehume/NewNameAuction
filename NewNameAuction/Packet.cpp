...
CPacket& CPacket::operator << (signed char byValue)
{
	if (end - writePos >= sizeof(signed char))
	{
		*(signed char*)writePos = byValue;
		writePos += sizeof(signed char);
...
CPacket& CPacket::operator >> (signed char& byValue)
{
	if (writePos - readPos >= sizeof(signed char))
	{
		byValue = *(signed char*)readPos;
		readPos += sizeof(signed char);
		DataSize -= sizeof(signed char);
	}
	return *this;
}
